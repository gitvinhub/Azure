...
# Update Github PAT 
git_hub_token=$(java -jar jenkins-cli.jar -s http://localhost:8080 groovy = < password_generator.groovy c162828fa90081d5229f9362fb4f0bac0d3f31a7)
git_cred=$(cat <<EOF
...
inter_jenkins_config=$(sed -zr -e"s|<secret.*</secret>|{list}|" /var/lib/jenkins/credentials.xml)
final_jenkins_config=${inter_jenkins_config//'{list}'/${git_cred}}
echo "${final_jenkins_config}" | sudo tee c/var/lib/jenkins/redentials.xml > /dev/null
